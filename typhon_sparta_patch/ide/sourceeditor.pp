...
uses
  ...
  MemCheck,
  {$ENDIF}
  SynEditMouseCmds, Classes, SysUtils, Math, Controls,

  ct_basefunctions, ct_emb_basics, ct_emb_formeditor, ct_emb_notebook,      //------------ ct9999 ----------
...
{ TSourceEditor ---
  ...
  public
    FParentTabPage:TEmbedTabSheet;  //-------------- ct9999 --------
  ...
  private
    FNotebook: TEmbedNotebook;                   //----------- ct9999 ----------
    FOnToggleFormOrSource:TNotifyEvent;          //----------- ct9999 ----------
    FBaseCaption: String;
    FIsClosing: Boolean;
  ...
  public
    property OnToggleFormOrSource:TNotifyEvent read FOnToggleFormOrSource write FOnToggleFormOrSource;  //----------- ct9999 ----------
...
var
  ...
  CT_LAST_SOURCE_EDITOR:TSourceEditor=nil; // ct9999
implementation
...
//-------------------------------------------  ct9999
FParentTabPage:=TEmbedTabSheet(AParent);

if CT_USE_EMBEDDED_DESIGNER then
begin

CT_LAST_SOURCE_EDITOR:=self;

 if FEditor.Parent is TEmbedTabSheet then
   begin
     TEmbedTabSheet(FEditor.Parent).FormEditorDelete;
   end;
end;
//------------------------------------------
end;
...
//------------------------------------------------------- ct9999  8888
// Reset Informations to EMBEDDED_DESIGNER Sub-System
// MUST ACTIONS
if CT_USE_EMBEDDED_DESIGNER then
 if FParentTabPage<>nil then
   begin
     FParentTabPage.OnToggleFormOrSource:=nil;  // UnLink Page OnToggleFormOrSource Event
     FParentTabPage.FormEditorDelete;
     FParentTabPage.FSynEditor:=nil;
   end;
//------------------------------------------------
...
Begin
  ...
  {$ENDIF}
  FNotebook := TEmbedNotebook.Create(self);  //----------- ct9999 ----------
  {$IFDEF IDE_DEBUG}
  ...
  with FNotebook do Begin
    ...
    {$ENDIF}
    Align := alClient;
    APage:=TEmbedTabSheet.Create(FNotebook); //----------- ct9999 ----------
...
begin
  ...
  else if Source is {TExtendedNotebook} TEmbedNotebook then //----------- ct9999 ----------
    FNotebook.ShowTabs := True;
end;
...
//------------------------------------------------------------ ct9999
  if CT_USE_EMBEDDED_DESIGNER then
   if FActiveWindow<>nil then
    FActiveWindow.OnToggleFormOrSource:=self.OnToggleFormOrSource;
//------------------------------------------------------------
end;
...
Function  TSourceEditorManagerBase.GetDisplayState: TDisplayState;
begin
 if ActiveSourceWindow = nil then exit;
 Result:=TEmbedNotebook(TSourceNotebook(ActiveSourceWindow).FNotebook).DisplayState;
end;

Procedure TSourceEditorManagerBase.SetDisplayState(const val:TDisplayState);
begin
  if ActiveSourceWindow = nil then exit;

  if CT_USE_EMBEDDED_DESIGNER then
  TEmbedNotebook(TSourceNotebook(ActiveSourceWindow).FNotebook).DisplayState:=val;
end;
//==========================================================
...
begin
  ...
  with CodeTemplateModul do begin
    if FileExistsUTF8(EditorOpts.CodeTemplateFilename) then
      ...
      Filename:=GetTyphonDir+cnCodeTemplatesFile; // in directory typhon/files  //----------- ct9999 ----------
...
begin
  ...
  with FCodeTemplateModul do begin
    ...
    if not FileExistsCached(DCIFilename) then
      DCIFilename:=GetTyphonFilesDir+cnCodeTemplatesFile; // in directory typhon/files  //----------- ct9999 ----------
...
procedure TSourceEditorManager.EmbeddedForm(Sender: TObject; aForm: TCustomForm);
  var xPage:TEmbedTabSheet;
      xSE: TSourceEditor;
begin
 if CT_USE_EMBEDDED_DESIGNER=FALSE then Exit;
 if Embed_IsDesignForm(aForm)=false then exit;
 if SourceWindowCount<1 then exit;

 xPage:=nil;
 xSE:=nil;

 if xSE = nil then xSE:=ActiveEditor;
 if xSE = nil then xSE:=CT_LAST_SOURCE_EDITOR;
 if xSE = nil then
 begin
  ShowMessage('[DEV IDE ERROR 120 ] Can NOT Find SourceEditor to Embedded Form: '+aForm.Name);
  EXIT;
 end;

xPage:=xSE.FParentTabPage;
if xPage=nil then
begin
  ShowMessage('[DEV IDE ERROR 130 ] Can NOT Find TabPage to Embedded Form: '+aForm.Name);
  EXIT;
end;

xPage.FSynEditor:=xSE.FEditor;

if ActiveSourceWindow <> nil then
      TEmbedNotebook(ActiveSourceWindow.FNotebook).EmbeddedForm(Sender,aForm,xPage) else
      TEmbedNotebook(SourceWindows[0].FNotebook  ).EmbeddedForm(Sender,aForm,xPage);

if xSE.FSourceNoteBook <> nil then
       xPage.OnToggleFormOrSource:=xSE.FSourceNoteBook.OnToggleFormOrSource;  // Link Page OnToggleFormOrSource Event
