...
{$ENDIF}
  ...
  // IDE interface
  bs_IDEIntf, //=== ct9999 =======================
  ObjectInspector, PropEdits, PropEditUtils,
 ...
 //------------------------------- ct9999 for CodeTyphon studio
  ct_basefunctions, ct_popuppkgs, ct_popupcomppages,
  ctidedesignersettings,ct_emb_basics,
 //-------------------------------
  // main ide
...
type
  ...
  public
    ...
    procedure exPkgDDOpenButtonClick(Sender: TObject);
    //......................for CT ExtentIDE.......................
  end;
...
begin
  ...
  begin
    WriteHelp(GetTyphonVersionString+' '+lisSVNRevision+TyphonRevisionStr); //=== ct9999 =================
    exit;
  end;
...
// load environment, miscellaneous, editor and codetools options
  ...
  begin
    ...
    if StartedByStartLazarus then
      StartFile:=ExtractFilePath(StartFile)+'typhonstart'+GetExeExt; //=== ct9999 ==================
    {$IFDEF Windows}
...
begin
  ...
  NowVer:=GetTyphonVersionString;                  //=== ct9999 =================
  ...
  begin
    ...
    if OldVer='' then
      OldVer:=SimpleFormat(lisPrior, [GetTyphonVersionString]);   //=== ct9999 =================
    s:=SimpleFormat(lisWelcomeToLazarusThereIsAlreadyAConfigurationFromVe, 
       [GetTyphonVersionString, LineEnding+LineEnding, OldVer, LineEnding, GetPrimaryConfigPath] );   //=== ct9999 =================
    if IsUpgrade then
...
//==================== ct9999 ================================== for CodeTyphon Studio
  CT_USE_DOCKING_SYSTEM      := EnvironmentOptions.UseDockingSystem;
  CT_SEND_MSG_TO_MESSAGESWIN := EnvironmentOptions.SendMsgToMessagesWin;
  CT_USE_EMBEDDED_DESIGNER   := EnvironmentOptions.UseEmbeddedFormDesigner;
...
begin
  ...
  //======= ct9999 =============================
  if CT_USE_EMBEDDED_DESIGNER then
    SourceEditorManager.OnToggleFormOrSource:= @OnSrcNoteBookActivated;
  //======= ct9999 =============================
...
//=============================================== ct9999 8888  for CodeTyphon Studio
// too many IDE user mouse clicks for msgs...
// Send some no mtConfirmation msg to Messages Windows

function TMainIDE.OnIDEMessageDialog(const aCaption, aMsg: string;
                                     DlgType: TMsgDlgType; Buttons: TMsgDlgButtons;
                                     const HelpKeyword: string): Integer;
//........................................
function _FindButtonsNum:integer;
 var xbts:TMsgDlgBtn;
begin
  result:=0;
  for xbts:=Low(TMsgDlgBtn) to High(TMsgDlgBtn) do
    if TMsgDlgBtn(xbts) in Buttons then
       inc(result);
end;
//........................................
begin
result:=0;
if CT_SEND_MSG_TO_MESSAGESWIN=false then
begin
    Result:=MessageDlg(aCaption,aMsg,DlgType,Buttons,HelpKeyword);
    Exit;
end;

if _FindButtonsNum>1 then  // More than 2 buttons is Confirmation Msg
 begin
    Result:=MessageDlg(aCaption,aMsg,DlgType,Buttons,HelpKeyword);
 end else
 begin
  Case DlgType of
   mtWarning     :  MessagesView.AddCustomMessage(mluWarning, aMsg);
   mtError       :  MessagesView.AddCustomMessage(mluError  , aMsg);
   mtInformation :  MessagesView.AddCustomMessage(mluNote   , aMsg);
  else
    Result:=MessageDlg(aCaption,aMsg,DlgType,Buttons,HelpKeyword);
  end;
 end;
...
begin
  ...
  if Assigned(ComponentListForm) then ComponentListForm.UpdateVisibleEx(LastFormActivated); //=== ct9999 ==================
...
//===================== ct9999 For CodeTyphon
  MainIDEBar.pnlSpeedButtons.Visible:=ComponentPaletteVisible;
//=====================
end;
...
begin
//============================================ ct9999 For CodeTyphon
  SpeedButtonsVisible:=not MainIDEBar.tbStandard.Visible;
  MainIDEBar.itmViewIDESpeedButtons.Checked:=SpeedButtonsVisible;
  MainIDEBar.tbStandard.Visible:=SpeedButtonsVisible;
...
begin
  ...
  if CT_USE_EMBEDDED_DESIGNER then SourceEditorManager.DisplayState:=dsSource;  //=== ct9999 ================= 8888
end;
...
Begin
  Project1.Modified:=true;  //=== ct9999   to save and .lpi  Always
  DoSaveProject([]);
end;
...
var zPos: TPoint;
begin
  zPos:=point(fSelPageButton.Width,fSelPageButton.Height);
  zPos:=fSelPageButton.ClientToScreen(zPos);
  DlgPopupCompPages_Execute(zPos.x,zPos.y);
end;
//=============================================================
  ...
  begin
    ...
    try
      ...
      ExeName := AppendPathDelim(EnvironmentOptions.GetParsedLazarusDirectory)+
             DarwinStartlazBundlePath + 'typhonstart';                             //=== ct9999 ============
      {$else}
      ExeName := AppendPathDelim(EnvironmentOptions.GetParsedLazarusDirectory) +
        'typhonstart' + GetExecutableExt;                                          //=== ct9999 ============
      {$endif}
...
begin
  ...
  if Application.Terminated then begin
    if StartedByStartLazarus then
      ExitCode := ExitCodeRestartTyphonIDE //=== ct9999 ==================
    else
      StartStarter;
  end;
end;
...
begin
  if MainIDEBar = nil then Exit;
  if ToolStatus = itExiting then Exit;
  NewCaption := Format(lisLazarusEditorV, [GetTyphonVersionString]); //=== ct9999 =================

  //===================================== ct9999 for CodeTyphon Studio  7777
  xi:=pos('Typhon',NewCaption);
  if xi>0 then
  begin
  {$IFDEF CPU64}
    Insert('64',NewCaption,xi+6);
  {$ELSE}
    Insert('32',NewCaption,xi+6);
  {$ENDIF}
  end;
  //======================================
...
begin
  if CT_USE_EMBEDDED_DESIGNER then
  begin
      if SourceEditorManager.DisplayState <> dsSource then begin
...
begin
  ...
  //============================================== ct9999
  if CT_USE_EMBEDDED_DESIGNER=FALSE then
     DisplayState:=dsSource else
...
begin
  if LastFormActivated<>nil then
    if LastFormActivated.Designer<>nil then
      begin
        if LastFormActivated = (Sender as TDesigner).Form then exit;  //If the same form exit
        TDesigner(LastFormActivated.Designer).IsActive:=false;        //DeActivated old Designer
      end;
  DisplayState:= dsForm;
...
if CT_USE_EMBEDDED_DESIGNER=false then exit;

if LastFormActivated=nil then exit;
